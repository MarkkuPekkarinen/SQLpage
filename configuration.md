...
| `stmp_host`                                  |                                                              | SMTP server used by the `sqlpage.send_mail` function. Accepts only a host name or `host:port`; if no port is provided, SQLPage uses port 25. Set with `STMP_HOST` in the environment. |
| `stmp_username`                              |                                                              | Optional SMTP user name for `sqlpage.send_mail`. When set, SQLPage authenticates to `STMP_HOST` using this user name and `stmp_password`. |
| `stmp_password`                              |                                                              | Optional SMTP password for `sqlpage.send_mail` when `stmp_username` is set. |
